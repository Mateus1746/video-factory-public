...
class Renderer:
    def __init__(self, headless=True):
        ...
        self.particle_cache = {} # Key: (size, color_rgb_tuple) -> Surface
        self.glow_cache = {} # Key: (radius, color_rgb_tuple) -> Surface
        
        # Create background surface
        self.bg_surface = self._create_bg()
        
        # Pre-render static UI elements
        self.header_surf = pygame.Surface((self.width, 100), pygame.SRCALPHA)
        self.header_surf.fill((0, 0, 0, 180))

    def _get_particle_surf(self, size, color, alpha):
        key = (size, color)
        if key not in self.particle_cache:
            s = pygame.Surface((size*2, size*2), pygame.SRCALPHA)
            pygame.draw.circle(s, color, (size, size), size)
            self.particle_cache[key] = s
        
        # We need to handle alpha. Blitting with alpha modulation is fast.
        # But cached surface has full alpha. 
        # Making a copy to set alpha is slow. 
        # Strategy: Pre-render generic white circle, tint it? No, just keep simple cache for now
        # or just draw directly if alpha is needed.
        # For this fix, let's cache the BASE particle and use set_alpha (which is fast on hardware but software dummy?)
        # Actually, draw.circle with alpha color directly on main surface is invalid in pygame < 2.
        # Let's stick to creating surface for alpha BUT reuse if alpha is 255.
        
        surf = self.particle_cache[key]
        # To apply alpha to an image in pygame efficiently:
        s_copy = surf.copy() # Still a copy... 
        s_copy.set_alpha(alpha)
        return s_copy

    def _get_glow_surf(self, radius, color):
        key = (int(radius), color)
        if key not in self.glow_cache:
            s = pygame.Surface((int(radius*3), int(radius*3)), pygame.SRCALPHA)
            pygame.draw.circle(s, (*color, 60), (int(radius*1.5), int(radius*1.5)), int(radius*1.2))
    ...
    def render(self, engine):
        ...
        for p in self.particles[:]:
            ...
            # Optimization: Use cached surface with alpha
            p_surf = self._get_particle_surf(p_size, color, alpha)
        ...
        for s in self.shockwaves[:]:
            ...
            alpha = int(255 * s[4])
            rad = int(s[2])
            # Shockwaves are rare, no cache needed
        ...
        for z in engine.zombies:
            ...
            # Glow Check (Cached)
            z_glow = self._get_glow_surf(z.radius, color)
        ...
        # 5. HUD - Redesigned
        # Top Header (Cached)
        self.surface.blit(self.header_surf, (0, 0))
        ...
        # --- HYPE BAR (Next Upgrade) ---
        target = engine.next_purchase
        cost = target['cost']
        name = target['type'].upper().replace("_", " ")
        
        # Progress math
        prog = min(1.0, engine.money / cost)
        
        bar_w = 600
        bar_h = 30
        bar_x = (self.width - bar_w) // 2
        bar_y = 110 # Below header
        
        # Background
        pygame.draw.rect(self.surface, (40, 40, 60), (bar_x, bar_y, bar_w, bar_h), border_radius=5)
        # Fill
        if prog > 0:
            fill_color = (0, 255, 0) if prog >= 1.0 else (255, 200, 0)
            pygame.draw.rect(self.surface, fill_color, (bar_x, bar_y, int(bar_w * prog), bar_h), border_radius=5)
        # Border
        pygame.draw.rect(self.surface, (255, 255, 255), (bar_x, bar_y, bar_w, bar_h), 2, border_radius=5)
        
        # Text
        txt_target = self.font_small.render(f"NEXT: {name} (${cost})", True, (255, 255, 255))
        txt_rect = txt_target.get_rect(center=(self.width//2, bar_y + bar_h//2))
        # Shadow
        txt_shad = self.font_small.render(f"NEXT: {name} (${cost})", True, (0,0,0))
        self.surface.blit(txt_shad, (txt_rect.x+2, txt_rect.y+2))
        self.surface.blit(txt_target, txt_rect)
