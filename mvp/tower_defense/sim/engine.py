...
class GameEngine:
    ...
    def _handle_auto_buy(self):
        current_count = len(self.soldiers)
        
        # New buy logic prioritizing variety
        unit_choice = 'rifle'
        
        # Randomize slightly to avoid rigid build orders
        rand = random.random()
        
        if self.money >= config.SOLDIER_STATS['rocket']['cost'] and rand < 0.4:
             unit_choice = 'rocket'
        elif self.money >= config.SOLDIER_STATS['minigun']['cost'] and rand < 0.5:
             unit_choice = 'minigun'
        elif self.money >= config.SOLDIER_STATS['sniper']['cost'] and rand < 0.6:
             unit_choice = 'sniper'
        elif self.money >= config.SOLDIER_STATS['shotgun']['cost']:
             unit_choice = 'shotgun'

        cost = config.SOLDIER_STATS[unit_choice]['cost']
        ...
        if self.money >= cost:
            ...
            self.soldiers.append(Soldier(x, y, unit_choice))
